...
class PyTorchModel(nn.Module):
	def __init__(self, dg):
		...
		self.__train_dataset = data.TensorDataset(torch.from_numpy(dg.train_x.transpose(0, 3, 1, 2)).to(device), torch.from_numpy(dg.train_y).to(device))
		self.__test_dataset = data.TensorDataset(torch.from_numpy(dg.test_x.transpose(0, 3, 1, 2)).to(device), torch.from_numpy(dg.test_y).to(device))
