...
class TFModel(BaseModel):
	def __init__(self, dg):
		...
		with tf.variable_scope('network'):
			...
			with tf.variable_scope('dense'):
				...
				now = tf.layers.dense(now, units=128, activation='relu', kernel_initializer=tf.initializers.glorot_normal, name='dense')
				now = tf.layers.dropout(now, rate=0.25, training=self.__training, name='drop')

			with tf.variable_scope('output'):
				self.__output = tf.layers.dense(now, units=10, kernel_initializer=tf.initializers.glorot_normal, name='dense')
		...
		with tf.variable_scope('metrics'):
			self.__accuracy = tf.reduce_sum(tf.cast(tf.equal(tf.argmax(self.__y, axis=1), self.__prediction), tf.float32), name='accuracy')

		self.__loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=self.__y, logits=self.__output, name='loss'))
	...
	def fit(self, epochs, batch_size):
		self.__sess.run(self.__iter.initializer, feed_dict={self.__x_placeholder: self._dg.train_x, self.__y_placeholder: self._dg.train_y,
															self.__batch_size: batch_size})
		n = self._dg.train_y.shape[0]
		for i in range(epochs):
			loss, acc = 0, 0
			for j in tqdm(range(ceil(n / batch_size)), desc='Epoch {0}'.format(i+1)):
				l, a, _ = self.__sess.run((self.__loss, self.__accuracy, self.__step), feed_dict={self.__training: True})
				loss += l
				acc += a
	...
	def predict(self):
		batch_size = 256
		self.__sess.run(self.__iter.initializer, feed_dict={self.__x_placeholder: self._dg.test_x, self.__y_placeholder: self._dg.test_y,
															self.__batch_size: batch_size})
